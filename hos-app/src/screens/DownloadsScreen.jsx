...
const INDETERMINATE_STATUSES = new Set(['EXTRACTING','MERGING','COPYING']);
const ACTIVE_STATUSES = new Set(['DOWNLOADING','EXTRACTING','MERGING','COPYING']);

function TaskRow({ task, onCancel, onRetry, onInfo, onToggle }) {
  const [hovered, setHovered] = useState(false);
  const isIndeterminate = INDETERMINATE_STATUSES.has(task.status);
  const isDeterminate = !isIndeterminate;
  const canCancel = ACTIVE_STATUSES.has(task.status) || task.status === 'QUEUED';
  const canRetry  = task.status === 'FAILED' || task.status === 'NOT_FOUND';
  const canInfo   = !!task.error;
  const barColor  = task.status === 'COMPLETED' ? 'var(--success)' : task.status === 'FAILED' ? 'var(--error)' : 'var(--brand)';
      ...
      >
        <div style={{ padding:'11px 12px', display:'flex', alignItems:'center', gap:9, minWidth:0 }}>
          ...
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"
              style={{ transform: task.expanded ? 'rotate(90deg)' : 'none', transition:'transform .15s ease' }}>
              <path d="m9 6 6 6-6 6"/>
            </svg>
          </button>
          <SourcePill source={task.source} />
          <span style={{ flex:1, minWidth:0, fontSize:13, fontWeight:500, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{task.name}</span>
        </div>
        ...
        <div style={{ padding:'11px 10px' }}>
          {isIndeterminate ? (
            ...
            <>
              <ProgressBar value={task.progress} color={barColor} height={6} />
              <div style={{ marginTop:4, fontFamily:'var(--font-mono)', fontSize:10, color:'var(--text-muted)' }}>{task.progress.toFixed(1)}%</div>
            </>
          )}
        </div>
        <div style={{ padding:'11px 10px', fontFamily:'var(--font-mono)', fontSize:12, color:'var(--text-muted)' }}>{fmtBytes(task.downloadedBytes)}</div>
        <div style={{ padding:'11px 10px', fontFamily:'var(--font-mono)', fontSize:12, color: task.speed > 0 ? 'var(--brand)' : 'var(--text-dim)' }}>{fmtSpeed(task.speed)}</div>
        <div style={{ padding:'11px 10px', fontFamily:'var(--font-mono)', fontSize:12, color:'var(--text-muted)' }}>{fmtEta(task.etaSec)}</div>
        <div style={{ padding:'11px 10px', fontFamily:'var(--font-mono)', fontSize:11.5, color:'var(--text-dim)' }}>{task.createdAt}</div>
      ...
      {task.expanded && task.tracks && (
        <div style={{ padding:'4px 16px 12px 46px', background:'color-mix(in srgb, var(--surface-2) 60%, var(--surface))', display:'flex', flexDirection:'column', gap:2 }}>
          {task.tracks.map((tr, i) => {
            const tPct = Math.min(100, tr._p ?? (task.progress * tr.frac));
            const tColor = tr.kind === 'video' ? 'var(--brand)' : tr.kind === 'audio' ? 'var(--success)' : 'var(--text-dim)';
            return (
              <div key={i} style={{ display:'grid', gridTemplateColumns:'minmax(0,1fr) 110px 200px', alignItems:'center', gap:12, padding:'6px 8px', borderRadius:7 }}>
                <div style={{ display:'flex', alignItems:'center', gap:9, minWidth:0 }}>
                  <span style={{ fontFamily:'var(--font-mono)', fontSize:9, fontWeight:600, padding:'2px 6px', borderRadius:4, background:'var(--border)', color:'var(--text-muted)' }}>{tr.kind}</span>
                  <span style={{ fontSize:12, color:'var(--text-muted)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{tr.label}</span>
                </div>
                <span style={{ fontFamily:'var(--font-mono)', fontSize:11, color:'var(--text-dim)' }}>{fmtBytes(task.totalBytes * tr.frac)}</span>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                  <div style={{ flex:1, height:4, borderRadius:3, background:'var(--border)', overflow:'hidden' }}>
                    <div style={{ height:'100%', borderRadius:3, width:`${tPct}%`, background:tColor, transition:'width .4s linear' }} />
                  </div>
                  <span style={{ fontFamily:'var(--font-mono)', fontSize:10, color:'var(--text-dim)', width:38, textAlign:'right' }}>{tPct.toFixed(0)}%</span>
                </div>
              </div>
            );
          })}
      ...
      <div style={{ display:'flex', alignItems:'center', gap:14 }}>
        <div style={{ flex:1, minWidth:0 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, minWidth:0 }}>
            <SourcePill source={task.source} />
            <span style={{ flex:1, minWidth:0, fontSize:14.5, fontWeight:600, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis' }}>{task.name}</span>
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:18, marginTop:11 }}>
            <div style={{ flex:1, display:'flex', alignItems:'center', gap:11 }}>
              <ProgressBar value={task.progress} indeterminate={isIndeterminate} color={barColor} height={8} />
              <span style={{ fontFamily:'var(--font-mono)', fontSize:12, fontWeight:600, width:52, flexShrink:0 }}>{isIndeterminate ? '...' : task.progress.toFixed(1)+'%'}</span>
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:16, fontFamily:'var(--font-mono)', fontSize:12 }}>
              <span style={{ color:'var(--text-muted)' }}>{fmtBytes(task.downloadedBytes)} / {fmtBytes(task.totalBytes)}</span>
              <span style={{ color: task.speed > 0 ? 'var(--brand)' : 'var(--text-dim)', minWidth:64 }}>{fmtSpeed(task.speed)}</span>
              <span style={{ color:'var(--text-dim)', minWidth:56 }}>{fmtEta(task.etaSec)}</span>
            </div>
          </div>
        </div>
...
export default function DownloadsScreen({ tasks, diskFreeGB, onCancel, onRetry, onToggleExpanded, onClearCompleted, onAddToast }) {
  const [layout, setLayout] = useState('table');
  const [errorTask, setErrorTask] = useState(null);

  const activeCount = tasks.filter(t => ACTIVE_STATUSES.has(t.status)).length;
  const totalSpeed  = tasks.reduce((s, t) => s + (t.speed || 0), 0);

  const segBtn = (active, onClick, icon, label) => (
  ...
  return (
    <div style={{ padding:'18px 20px 36px' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:18, marginTop:9 }}>
            {totalSpeed > 0 && (
              <div style={{ display:'flex', alignItems:'center', gap:7, fontSize:13, fontWeight:600 }}>
                ...
                <span style={{ color:'var(--text-muted)', fontWeight:500 }}>Overall speed</span>
                <span style={{ fontFamily:'var(--font-mono)', color:'var(--brand)' }}>
                  {totalSpeed >= 1e6 ? (totalSpeed/1e6).toFixed(1)+' MB/s' : (totalSpeed/1e3).toFixed(0)+' KB/s'}
                </span>
              </div>
            )}
            <div style={{ display:'flex', alignItems:'center', gap:7, fontSize:12.5, color:'var(--text-muted)' }}>
              ...
              Free disk space <span style={{ fontFamily:'var(--font-mono)', color:'var(--text)', fontWeight:600, marginLeft:4 }}>{diskFreeGB.toFixed(1)} GB</span>
            </div>
          </div>
        </div>
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ display:'flex', padding:3, gap:2, borderRadius:9, background:'var(--surface-2)', border:'1px solid var(--border)' }}>
            {segBtn(layout==='table', () => setLayout('table'),
            ...
            {segBtn(layout==='cards', () => setLayout('cards'),
            ...
            <div style={{ minWidth:1080 }}>
              ...
              {tasks.map(t => (
                <TaskRow
                  ...
                  onInfo={setErrorTask} onToggle={onToggleExpanded}
                />
              ))}
            </div>
          </div>
      ...
      {/* Error dialog */}
      {errorTask && (
        <div onClick={() => setErrorTask(null)} style={{
          position:'fixed', inset:0, zIndex:60,
          background:'rgba(3,5,9,.62)', backdropFilter:'blur(3px)',
          display:'grid', placeItems:'center', padding:24,
        }}>
          <div onClick={e => e.stopPropagation()} style={{
            width:520, maxWidth:'94vw', maxHeight:'88vh', overflow:'hidden',
            ...
            <div style={{ padding:20 }}>
              <div style={{ display:'flex', gap:13 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <p style={{ margin:0, fontFamily:'var(--font-mono)', fontSize:12.5, lineHeight:1.55, color:'var(--text-muted)', wordBreak:'break-word' }}>{errorTask.error}</p>
                </div>
              </div>
            </div>
          </div>
